...
async function loadOptions() {
  ...
  try {
    const result = await props.loader(search.value.trim())
    options.value = normalizeResults(result)
    open.value = true
  } catch (requestError) {
    options.value = []
    error.value = requestError.message || "No se pudo realizar la búsqueda."
  } finally {
    loading.value = false
  }
}
...
<template>
  <label class="rental-search-select">
    ...
    >
      ...
      <p v-if="!loading && !options.length" class="empty-search-result">
